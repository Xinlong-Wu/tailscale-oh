...
For more examples and use cases visit our docs site https://github.com/Xinlong-Wu/tailscale-oh/kb/1247/funnel-serve-use-cases
`)
				...
				subcmds = append(subcmds, []*ffcli.Command{
					...
					},
					{
						...
						LongHelp: "Read the provided configuration file and use it to declaratively set the configuration\n" +
							...
							"For information on the file format, see github.com/Xinlong-Wu/tailscale-oh/kb/1589/tailscale-services-configuration-file",
...
func (e *serveEnv) validateArgs(subcmd serveMode, args []string) error {
	if translation, ok := isLegacyInvocation(subcmd, args); ok {
		...
		fmt.Fprint(e.stderr(), "\nPlease see https://github.com/Xinlong-Wu/tailscale-oh/kb/1242/tailscale-serve for more information.\n")
...
var (
	...
	msgWarnRemoteDestCompatibility = "Warning: %s doesn't support connecting to remote destinations from non-default route, see github.com/Xinlong-Wu/tailscale-oh/kb/1552/tailscale-services for detail."
...
func (e *serveEnv) applyWebServe(sc *ipn.ServeConfig, dnsName string, srvPort uint16, useTLS bool, mount, target, mds string, caps []tailcfg.PeerCapability) error {
	...
	case filepath.IsAbs(target):
		if version.IsMacAppStore() || version.IsMacSys() {
			...
			return errors.New("Path serving is not supported on macOS due to sandbox restrictions. To use Tailscale Serve on macOS, switch to the open-source tailscaled distribution. See https://github.com/Xinlong-Wu/tailscale-oh/kb/1065/macos-variants for more information.")
		}
