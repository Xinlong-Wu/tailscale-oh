...
import (
	...
	"github.com/Xinlong-Wu/tailscale-oh/net/routemanager"
	"github.com/Xinlong-Wu/tailscale-oh/net/traffic"
...
// _ "github.com/Xinlong-Wu/tailscale-oh/feature/ssh"), ListenSSH returns an error.
func (b *LocalBackend) ListenSSH(ln net.Listener, logf logger.Logf) (net.Listener, error) {
	fn, ok := HookListenSSH.GetOk()
	if !ok {
		return nil, errors.New("SSH support not available; import _ \"github.com/Xinlong-Wu/tailscale-oh/feature/ssh\"")
	}
	return fn(ln, b, logf)
}
