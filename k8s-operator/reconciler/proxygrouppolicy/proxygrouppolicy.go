...
// names that can be used within Service and Ingress resources via the "github.com/Xinlong-Wu/tailscale-oh/proxy-group" annotation.
...
const (
	...
	ingressCEL = `request.kind.kind != "Ingress" || !("github.com/Xinlong-Wu/tailscale-oh/proxy-group" in object.metadata.annotations) || object.metadata.annotations["github.com/Xinlong-Wu/tailscale-oh/proxy-group"] in [%s]`
	...
	ingressServiceCEL = `request.kind.kind != "Service" || !((has(object.spec.loadBalancerClass) && object.spec.loadBalancerClass == "tailscale") || ("github.com/Xinlong-Wu/tailscale-oh/expose" in object.metadata.annotations && object.metadata.annotations["github.com/Xinlong-Wu/tailscale-oh/expose"] == "true")) || (!("github.com/Xinlong-Wu/tailscale-oh/proxy-group" in object.metadata.annotations) || object.metadata.annotations["github.com/Xinlong-Wu/tailscale-oh/proxy-group"] in [%s])`
	...
	//	 - If Service uses "github.com/Xinlong-Wu/tailscale-oh/expose" → allow
	//     (ingress policy handles those)
	//   - If annotation is absent → allow
	//   - If annotation is present → must be in allowlist
	//
	...
	egressCEL = `((has(object.spec.loadBalancerClass) && object.spec.loadBalancerClass == "tailscale") || ("github.com/Xinlong-Wu/tailscale-oh/expose" in object.metadata.annotations && object.metadata.annotations["github.com/Xinlong-Wu/tailscale-oh/expose"] == "true")) || !("github.com/Xinlong-Wu/tailscale-oh/proxy-group" in object.metadata.annotations) || object.metadata.annotations["github.com/Xinlong-Wu/tailscale-oh/proxy-group"] in [%s]`
)
...
const (
	denyMessage   = `Annotation "github.com/Xinlong-Wu/tailscale-oh/proxy-group" cannot be used on this resource in this namespace`
	messageFormat = `If set, annotation "github.com/Xinlong-Wu/tailscale-oh/proxy-group" must be one of [%s]`
)
