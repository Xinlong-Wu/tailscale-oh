...
$bootstrapScriptBlock = {
    ...
    if (-not $gocrossOk) {
        ...
        $proc = Start-Process -FilePath $procExe -WorkingDirectory $repoRoot -Environment $goBuildEnv -ArgumentList 'build', '-o', $gocrossPath, "-ldflags=-X=github.com/Xinlong-Wu/tailscale-oh/version.gitCommitStamp=$wantVer", 'github.com/Xinlong-Wu/tailscale-oh/tool/gocross' -NoNewWindow -PassThru
        $proc.WaitForExit()
